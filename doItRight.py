...
def find_best_move(last_player_chosen_node, last_player_Id, chosen_node_depth, score_including_chosen_node, visited, frontier, a, b):
	


	# playerId = playerId%2+1
	# player = players[playerId] # player to make next move
	...
	# val = '-inf'
	# point = player.preference[node.color]

	# if player.id == 2:
	# 	val = 'inf'
	# 	point = -point

	last_player = players[last_player_Id]
	next_player = players[last_player_Id%2+1]

	val = '-inf'
	# point = last_player.preference[last_player_chosen_node.color]
	...
	if next_player.id == 2:
		val = 'inf'
		# point = -point
	if chosen_node_depth == 3:
		val = score_including_chosen_node
	print_log(last_player_chosen_node, chosen_node_depth, val, a, b)
	...
	for node in sorted_frontier:
		...
		for color in sorted_non_adjacent_colors:
			...
			point = next_player.preference[color]
			if next_player.id==2:
				point = -point
			...
			score = find_best_move(node, next_player.id, chosen_node_depth+1, score_including_chosen_node+point, visited, frontier, a, b)
			# score = 0
			# if next_node: 
			...
			if next_player.id == 1:
				...
				if val == '-inf' or score > val:
					...
					best_node = node
					best_color = color
				if (b=='inf' or val<b )and (a == '-inf' or val > a):
					a = val
				if val >= b:
					should_break = True
					# print_log(last_player_chosen_node, chosen_node_depth, val, a, b)
				...
				if val=='inf' or score < val:
					...
					best_color = color
					# b = min(b, val)
				if (a == '-inf' or val>a) and (b == 'inf' or val < b):
					b = val
				if val <= a:
					should_break = True
					# print_log(node, depth, val, a, b)
...
with open("testcases/t5.txt", 'r') as f:
	...
	best_first_move = find_best_move(parent, 2, 0, initial_score, visited, frontier, '-inf', 'inf')
